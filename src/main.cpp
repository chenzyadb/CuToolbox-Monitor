...
int main(int argc, char* argv[])
{
    ...
    if (!IsPathExist("/sys/class/power_supply/battery") && IsPathExist("/sys/class/power_supply/Battery")) {
    ...
    for (;;) {
        ...
        }
        {
            ...
            // #MEM Free 1919 MB (18%)
            outputLines.emplace_back(StrMerge("#MEM Free %d MB (%d%%)", memAvailable / 1024, memFreePercentage));
        }
        {
