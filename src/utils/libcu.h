...
inline std::string DumpTopActivityInfo(void) 
{
...
inline int GetTaskType(const int &pid) noexcept
{
...
inline uint64_t GetThreadRuntime(const int &pid, const int &tid) noexcept
{
    ...
    if (fd >= 0) {
        ...
        sscanf((strchr(buffer, ')') + 2), "%*c %*lu %*lu %*lu %*lu %*lu %*lu %*lu %*lu %*lu %*lu %" SCNu64 " %" SCNu64, &utime, &stime);
        return (utime + stime);
    }
    return 0;
}

inline int GetScreenStateViaCgroup(void) noexcept
{
...
inline int GetScreenStateViaWakelock(void) noexcept
{
...
inline int GetCompileDateCode(const std::string &compileDate)
{
...
inline int GetAndroidSDKVersion(void) noexcept
{
...
inline std::string GetDeviceSerialNo(void) 
{
    char buffer[PROP_VALUE_MAX] = { 0 };
...
inline int GetLinuxKernelVersion(void) noexcept
{
...
inline uint64_t GetTimeStampMs(void) noexcept
{
    struct timespec ts{};
...
inline int StringToInteger(const std::string &str) noexcept
{
    ...
    for (int pos = 0; pos < str.length(); pos++) {
        ...
        if (!negative && ret > INT32_MAX) {
            return INT32_MAX;
        } else if (negative && -ret < INT32_MIN) {
            return INT32_MIN;
        }
    }
    if (negative) {
